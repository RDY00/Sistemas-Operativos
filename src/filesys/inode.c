...
/* TODO:
   - Change inode_disk UNUSED length based on new inode size (size of list).
   - Inicialize file_sectors list (PROB in create_inode) DONE
   - Change functions to work with lists instead of arrays.
     * byte_to_sector DONE
     * inode_create
     * inode_close
     * inode_open (?)
...
//bool get_free_map_sectors (struct inode *inode, size_t size);
...
byte_to_sector (const struct inode *inode, off_t pos)
{
  ...
  if (pos < inode->data.length) {
    struct list_elem *e = list_begin (inode->data->file_sectors);
  ...
  if (disk_inode != NULL)
    {
      ...
      list_init(&disk_inode->file_sectors);
